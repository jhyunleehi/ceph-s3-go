...
func (suite *RadosTestSuite) TestCreate() {
	...
	err := suite.ioctx.Create("unique", rados.CreateExclusive)
	assert.NoError(suite.T(), err)

	err = suite.ioctx.Create("unique", rados.CreateExclusive)
	assert.Error(suite.T(), err)
	assert.Equal(suite.T(), err, rados.ErrObjectExists)

	err = suite.ioctx.Create("unique", rados.CreateIdempotent)
	assert.NoError(suite.T(), err)
}
...
func (suite *RadosTestSuite) TestReadNotFound() {
	...
	assert.Equal(suite.T(), err, rados.ErrNotFound)
}
...
func (suite *RadosTestSuite) TestObjectIteratorAcrossNamespaces() {
	...
	suite.ioctx.SetNamespace(rados.AllNamespaces)
	existingList := []string{}
	iter, err := suite.ioctx.Iter()
	assert.NoError(suite.T(), err)
	for iter.Next() {
...
func (suite *RadosTestSuite) TestSetGetNamespace() {
	...
	suite.T().Run("allNamespaces", func(t *testing.T) {
		...
		ns, err := suite.ioctx.GetNamespace()
		assert.NoError(suite.T(), err)
		assert.Equal(suite.T(), rados.AllNamespaces, ns)
	})
